...
namespace Game1.States
{
    public class EngineState : State
    {
        ...
        public EngineState(Game1 game, GraphicsDevice graphicsDevice, ContentManager Content, string levelName = "")
          : base(game, graphicsDevice, Content)
        {
            ...
            {
                powerUpSpawner.Load(powerUpSpawner.PowerUpType, powerUpSpawner.Location, powerUpTexture);
            }
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            {
                successSoundEffect.Play(soundLevel * 2f, 0f, 0f);
                _game.ReloadLevels();
            ...
            else
            {
                ...
                if (GamePad.GetState(PlayerIndex.One).Buttons.A == ButtonState.Pressed && PointIsEmpty(cursorLocation))
                {
                    level.BlockMap[cursorLocation] = new TerrainBlock(
                         terrainPallete, spriteLocation.ToVector2(), cursorLocation.ToVector2() * Constants.tileSize, 16, true);
                    inputDelay = 6;
                    successSoundEffect.Play(soundLevel * 2f, 0f, 0f);
                }
                ...
                {
                    level.BlockMap[cursorLocation] = new TerrainBlock(
                         terrainPallete, spriteLocation.ToVector2(), cursorLocation.ToVector2() * Constants.tileSize, 16, false);
                    successSoundEffect.Play(soundLevel * 2f, 0f, 0f);
                    inputDelay = 6;
                }
                else if (GamePad.GetState(PlayerIndex.One).Buttons.Y == ButtonState.Pressed && PointIsEmpty(cursorLocation))
                {
                    level.BackgroundBlockMap[cursorLocation] = new TerrainBlock(
                         terrainPallete, spriteLocation.ToVector2(), cursorLocation.ToVector2() * Constants.tileSize, 16, false, true);
                    successSoundEffect.Play(soundLevel * 2f, 0f, 0f);
                    inputDelay = 6;
                }
                ...
                else if (GamePad.GetState(PlayerIndex.One).DPad.Left == ButtonState.Pressed)
                {
                    if (PointIsEmpty(cursorLocation))
                    {
                        ...
                        powerUpSpawner.Load(PowerUpType.Health, cursorLocation, powerUpTexture);
                ...
                else if (GamePad.GetState(PlayerIndex.One).DPad.Right == ButtonState.Pressed)
                {
                    ...
                    {
                        var powerUpSpawner = new PowerUpSpawner();
                        powerUpSpawner.Load(PowerUpType.RocketAmmo, cursorLocation, powerUpTexture);
                        level.PowerUpSpawners.Add(powerUpSpawner);
                        successSoundEffect.Play(soundLevel * 2f, 0f, 0f);
                    }
                    inputDelay = 6;
